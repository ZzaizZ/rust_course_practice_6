...
mod stdp { // parsers for std types
    ...
    impl Parser for I32 {
        ...
        fn parse<'a>(&self, input: &'a str) -> Result<(&'a str, Self::Dest), ()> {
            ...
            Ok((&input[end_idx..], NonZeroI32::new(value).ok_or(())?))
        }
    }
...
impl<T: Parser> Parser for StripWhitespace<T> {
    type Dest = T::Dest;
    fn parse<'a>(&self, input: &'a str) -> Result<(&'a str, Self::Dest), ()> {
        self.parser.parse(input.trim_start()).map(
...
impl Parsable for AuthData {
    ...
    fn parser () -> Self::Parser {
        map(take(AUTHDATA_SIZE, stdp::Byte), |authdata| AuthData(authdata.try_into().unwrap_or(vec![0; AUTHDATA_SIZE])))
    }
}
...
pub enum AppLogJournalKind {
    CreateUser{user_id: String, authorized_capital: NonZeroU32},
    DeleteUser{user_id: String},
    RegisterAsset{asset_id: String, user_id: String, liquidity: NonZeroU32},
...
impl Parsable for AppLogJournalKind {
    type Parser = Preceded<Tag, Alt<(
        Map<Preceded<StripWhitespace<Tag>,Delimited<Tag,Permutation<(KeyValue<Unquote>,KeyValue<stdp::U32>)>,Tag>>,fn((String,NonZeroU32))->AppLogJournalKind>,
        Map<Preceded<StripWhitespace<Tag>,Delimited<Tag,KeyValue<Unquote>,Tag>>,fn(String)->AppLogJournalKind>,
        Map<Preceded<StripWhitespace<Tag>,Delimited<Tag,Permutation<(KeyValue<Unquote>,KeyValue<Unquote>,KeyValue<stdp::U32>)>,Tag>>,fn((String,String,NonZeroU32))->AppLogJournalKind>,
...
mod test {
    ...
    #[test]
    fn test_u32() {
        assert_eq!(stdp::U32.parse("411".into()), Ok(("".into(), NonZeroU32::new(411).unwrap())));
        assert_eq!(stdp::U32.parse("411ab".into()), Ok(("ab".into(), NonZeroU32::new(411).unwrap())));
        assert_eq!(stdp::U32.parse("".into()), Err(()));
        assert_eq!(stdp::U32.parse("-3".into()), Err(()));
        assert_eq!(stdp::U32.parse("0x03".into()), Ok(("".into(), NonZeroU32::new(0x3).unwrap())));
        assert_eq!(stdp::U32.parse("0x03abg".into()), Ok(("g".into(), NonZeroU32::new(0x3ab).unwrap())));
        assert_eq!(stdp::U32.parse("0x".into()), Err(()));
    }

    #[test]
    fn test_i32() {
        assert_eq!(stdp::I32.parse("411".into()), Ok(("".into(), NonZeroI32::new(411).unwrap())));
        assert_eq!(stdp::I32.parse("411ab".into()), Ok(("ab".into(), NonZeroI32::new(411).unwrap())));
        assert_eq!(stdp::I32.parse("".into()), Err(()));
        assert_eq!(stdp::I32.parse("-3".into()), Ok(("".into(), NonZeroI32::new(-3).unwrap())));
    ...
    fn test_strip_whitespace() {
        ...
        assert_eq!(strip_whitespace(stdp::U32).parse(" 42 answer".into()), Ok(("answer".into(), NonZeroU32::new(42).unwrap())));
    }

    #[test]
    fn test_delimited() {
        assert_eq!(delimited(tag("["), stdp::U32, tag("]")).parse("[0x32]".into()), Ok(("".into(), NonZeroU32::new(0x32).unwrap())));
        assert_eq!(delimited(tag("[".into()), stdp::U32, tag("]".into())).parse("[0x32] nice".into()), Ok((" nice".into(), NonZeroU32::new(0x32).unwrap())));
    ...
    #[test]
    fn test_key_value() {
        assert_eq!(key_value("key", stdp::U32).parse(r#""key":32,"#.into()), Ok(("".into(), NonZeroU32::new(32).unwrap())));
        assert_eq!(key_value("key", stdp::U32).parse(r#"key:32,"#.into()), Err(()));
        assert_eq!(key_value("key", stdp::U32).parse(r#""key":32"#.into()), Err(()));
        assert_eq!(key_value("key", stdp::U32).parse(r#" "key" : 32 , nice"#.into()), Ok(("nice".into(), NonZeroU32::new(32).unwrap())));
    }

    #[test]
    fn test_list() {
        assert_eq!(list(stdp::U32).parse("[1,2,3,4,]".into()), Ok(("".into(), vec![NonZeroU32::new(1).unwrap(), NonZeroU32::new(2).unwrap(), NonZeroU32::new(3).unwrap(), NonZeroU32::new(4).unwrap()])));
        assert_eq!(list(stdp::U32).parse(" [ 1 , 2 , 3 , 4 , ] nice".into()), Ok(("nice".into(), vec![NonZeroU32::new(1).unwrap(), NonZeroU32::new(2).unwrap(), NonZeroU32::new(3).unwrap(), NonZeroU32::new(4).unwrap()])));
    ...
    #[test]
    fn test_backet() {
        assert_eq!(Backet::parser().parse(r#"Backet{"asset_id":"usd","count":42,}"#.into()), Ok(("".into(), Backet{asset_id: "usd".into(), count: NonZeroU32::new(42).unwrap()})));
        assert_eq!(Backet::parser().parse(r#"Backet{"count":42,"asset_id":"usd",}"#.into()), Ok(("".into(), Backet{asset_id: "usd".into(), count: NonZeroU32::new(42).unwrap()})));
    }
    ...
    fn test_log_kind() {
        ...
        assert_eq!(LogKind::parser().parse(r#"App::Journal RegisterAsset {"asset_id": "bayc", "liquidity": 100000000, "user_id": "Steeve",}"#.into()), Ok(("".into(), LogKind::App(AppLogKind::Journal(AppLogJournalKind::RegisterAsset{asset_id: "bayc".into(), user_id: "Steeve".into(), liquidity: NonZeroU32::new(100_000_000).unwrap()})))));
        assert_eq!(LogKind::parser().parse(r#"App::Journal DepositCash UserCash{"user_id": "Steeve", "count": 10,}"#.into()), Ok(("".into(), LogKind::App(AppLogKind::Journal(AppLogJournalKind::DepositCash(UserCash{user_id: "Steeve".into(), count: NonZeroU32::new(10).unwrap()}))))));
        assert_eq!(LogKind::parser().parse(r#"App::Journal BuyAsset UserBacket{"user_id": "Steeve", "backet": Backet{"asset_id":"bayc","count":1,},}"#.into()), Ok(("".into(), LogKind::App(AppLogKind::Journal(AppLogJournalKind::BuyAsset(UserBacket{user_id: "Steeve".into(), backet: Backet{asset_id: "bayc".into(), count: NonZeroU32::new(1).unwrap()}}))))));
    }
}
